...
{
	// yuv数据转rgb
    public native static void yuv2rgb(int handle, int[] rgb, int width, int height);
    // yuv数据拷贝
    public native static void copyYUVData(int srcHandle, byte[] dst_y, byte[] dst_u, byte[] dst_v, int length);
    // 普通帧数据拷贝
    public native static void copyFrameData(int srcHandle, byte[] dstData, int length);
	// yuv数据裁剪并旋转
    ...
    // 保存/删除数据帧
    public native static int  saveDataFrame(byte[] data, int length);
    public native static void deleteDataFrame(int handle);
    // 读取帧数据
