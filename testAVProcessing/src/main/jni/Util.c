...
// 保存帧数据
JNIEXPORT jint JNICALL Java_com_xiuxiu_util_AVProcessing_saveDataFrame(JNIEnv *env, jclass obj, jbyteArray data, jint length)
{
...
// 删除帧数据
JNIEXPORT void JNICALL Java_com_xiuxiu_util_AVProcessing_deleteDataFrame(JNIEnv *env, jclass obj, jint handle)
{
	int *pHandle = (int*)handle;
	if (pHandle != NULL)
	{
		free(pHandle);
		pHandle = NULL;
...
// 拷贝帧数据
JNIEXPORT void JNICALL Java_com_xiuxiu_util_AVProcessing_copyFrameData(JNIEnv *env, jclass obj, jint handle, jbyteArray dstData, jint length)
{
